import first_task_4.{Animal, AnimalStore, BankService}
...
object Main extends App {
  ...
  try {
    ...
    if (new AnimalStore(List(Animal("Кот", 50.0),Animal("Собака", 100.0))).buyAnimal("Кот")._1 != null) {
      printAnimal(new AnimalStore(List(Animal("Кот", 50.0),Animal("Собака", 100.0))).buyAnimal("Кот")._1.name)
      BankService(1000.0).deductFunds(new AnimalStore(List(Animal("Кот", 50.0),Animal("Собака", 100.0))).buyAnimal("Кот")._2).showBalance
    }
    else printNotFoundAnimal(new AnimalStore(List(Animal("Кот", 50.0),Animal("Собака", 100.0))).buyAnimal("Кот")._1.name)
  }
  catch {
